...
- **2026-09-13 — Routing-Anonymität (Ausgangsfrage: "was gibt ein Message-Versand über Sender/Empfänger/Inhalt preis") analysiert, ein Teil umgesetzt.** Siehe `docs/routing-anonymity.md`: Sender-Anonymität für self-certifying Kinds ist jetzt fertig (`@qu/bootstrap`'s `bootstrapAliasSpace()`, komponiert aus `alias.js`'s bereits vorhandenem `publishAlias()`). Zwei weitergehende Vorschläge (Empfänger-Set-Padding in `envelope.to` für Gruppen-Writes; epoch-basierte Alias-Rotation mit Korrespondenz-Handoff) sind bewusst nur als Design dokumentiert — sie ändern das Envelope-Format bzw. führen neues Protokollverhalten ein und warten auf explizite Freigabe.
